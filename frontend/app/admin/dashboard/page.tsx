...
interface AdminUser {
  ...
  name: string;
  email: string;
  role: "organizer" | "participant" | "vendor" | "admin";
...
function AdminDashboard() {
  ...
  const fetchAll = () => {
    ...
    setError("");
    Promise.all([
      apiList<AdminUser>("/api/users"),
      apiList<EscrowRow>("/api/admin/escrow"),
    ])
      .then(([u, e]) => {
        setUsers(u);
        setEscrow(e);
  ...
  const handleToggleBan = async (user: AdminUser) => {
    ...
    if (nextStatus === "banned" && !confirm(`Ban ${user.name || user.email}? They lose access immediately.`)) return;

    try {
      await api(`/api/users/${user._id}/status`, {
        method: "PUT",
  ...
  const handleDisburse = async (eventId: string) => {
    try {
      await api(`/api/events/${eventId}/disburse`, { method: "PUT" });
      fetchAll();
    } catch (err) {
  ...
  const handleDisburseAll = async () => {
    ...
    if (!confirm(`Release escrow for ${held.length} event(s), ₹${totalEscrow.toLocaleString("en-IN")} in total? This cannot be undone.`)) return;

    // Report partial failures instead of silently swallowing them — the old
    // version fired every request and refreshed regardless of the outcome.
    const results = await Promise.allSettled(
      held.map((p) => api(`/api/events/${p.eventId}/disburse`, { method: "PUT" }))
    );
    const failed = results.filter((r) => r.status === "rejected").length;
    if (failed > 0) setError(`${failed} of ${held.length} disbursements failed. The rest went through.`);
    fetchAll();
  };
                ...
                {users.length === 0 ? (
                  ...
                  <div key={user._id} className="flex items-center gap-3 px-6 py-4 hover:bg-white/[0.02] transition-colors">
                    ...
                    <div className="flex flex-col items-end gap-1">
                      ...
                      {/* Banning yourself locks you out of this page, so the
                          server rejects it — don't offer the button either. */}
                      {user.descopeId !== me?.id && (
                        <button
