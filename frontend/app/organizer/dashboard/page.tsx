...
function ProgressBar({ value, max }: { value: number; max: number }) {
  // max can legitimately be 0 (an event with no seats), which would render
  // width: NaN% and leave the bar stuck full.
...
function CreateEventModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    // `!form.price` rejected "0", so a free event could never be created.
    ...
    if (!Number.isFinite(seats) || seats <= 0) {
      setError("Total seats must be a number greater than zero.");
      return;
    }
...
function DeleteModal({ event, onClose, onDeleted }: { event: Event; onClose: () => void; onDeleted: () => void }) {
  ...
  const handleDelete = async () => {
    try {
      await api(`/api/events/${getId(event)}`, { method: "DELETE" });
      onDeleted();
      onClose();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center">
      <div className="bg-surface border border-white/[0.1] rounded-2xl p-8 w-full max-w-sm shadow-2xl">
        ...
        <p className="text-muted text-sm mb-6"><span className="text-white font-medium">{event.title}</span> will be permanently deleted.</p>
        {error && (
          <div className="mb-4 p-3 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 text-xs">
            {error}
          </div>
...
function EditModal({ event, onClose, onUpdated }: { event: Event; onClose: () => void; onUpdated: () => void }) {
  ...
  const handleUpdate = async () => {
    ...
    const open  = Number(form.availableSeats);
    const price = form.price === "" ? 0 : Number(form.price);
    ...
    if (!Number.isFinite(open) || open < 0 || open > total) {
    ...
    setError("");
    try {
      await api(`/api/events/${getId(event)}`, {
        method: "PUT",
        body: JSON.stringify({
          title:          form.title,
          date:           form.date,
...
function OrganizerDashboard() {
  ...
  const fetchEvents = () => {
    setLoading(true);
    // ?mine=true — the unscoped list showed every organizer's events, with
    // Edit/Delete buttons that could only ever 403.
    apiList<Event>("/api/events?mine=true")
      .then(setEvents)
      .catch((err) => setLoadError(err instanceof Error ? err.message : "Could not load your events."))
      .finally(() => setLoading(false));
  };

  const fetchRegistrations = () => {
    // Every registration across this organizer's events. The old
    // /registrations/recent is capped at 5 rows, which made the revenue
    // headline wrong for anyone with a sixth registration.
    apiList<Registration>("/api/registrations/organizer")
  ...
  return (
    <div className="min-h-screen bg-bg flex font-dm text-[var(--text)]">
      ...
      <main className="flex-1 ml-64 transition-all duration-300 min-h-screen">
        ...
        <Header username="Dubey" role="organizer" showNewEventButton={true} onNewEvent={() => setShowCreate(true)} />
