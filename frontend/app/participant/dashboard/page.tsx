...
import { api, apiList, getId } from "@/lib/api";
...
// ─── Edit Profile Modal ──────────────────────────────────────────────
function EditProfileModal({ onClose, onUpdated }: { onClose: () => void; onUpdated: () => void }) {
  const user = getUser();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (user) saveUser({ ...user, name, college });
      onUpdated();
      onClose();
    } catch (err) {
      ...
      <div className="bg-surface border border-white/[0.1] rounded-2xl p-8 w-full max-w-md shadow-2xl">
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              className="w-full bg-bg border border-white/[0.1] rounded-xl px-4 py-3 text-sm outline-none focus:border-accent transition-colors"
              value={name} onChange={(e) => setName(e.target.value)}
            />
          </div>
          <div>
            <label className="text-xs text-muted mb-1 block">College / Organization</label>
            <input
              className="w-full bg-bg border border-white/[0.1] rounded-xl px-4 py-3 text-sm outline-none focus:border-accent transition-colors"
...
function RegisterModal({ event, onClose, onRegistered }: {
  ...
  // The event's own price, full stop. Falling back to a hardcoded ₹499 here
  // charged people a number nobody had set.
  const amount                = event.price ?? 0;

  // A registration holds one of the event's seats the moment it's created.
  // If the user then walks away from the Razorpay sheet, that seat is gone
  // unless we hand it back.
  const releaseAbandoned = async (registrationId: string) => {
    try {
      await api(`/api/registrations/${registrationId}`, { method: "DELETE" });
      onRegistered();
    } catch {
      // Best effort — the server also sweeps stale unpaid registrations.
    }
  };
  ...
  const handlePayment = async () => {
    ...
    try {
      const registration = await api<{ _id: string; amount: number }>("/api/registrations", {
        method: "POST",
        body: JSON.stringify({
          participantName,
          eventId:  getId(event),
      ...
      // Branch on what the server actually charged, not on the price this
      // page loaded with — if the organizer raised it in the meantime, the
      // stale value would skip payment and leave the seat held but unpaid.
      ...
      const orderData = await api<{ orderId: string; amount: number; currency: string; keyId: string }>(
        "/api/payments/create-order",
        {
          method: "POST",
          // No amount here — the server reads it from the registration.
      ...
      if (!loaded) {
        ...
        await releaseAbandoned(registrationId);
        setLoading(false);
        return;
      }
      ...
      const options = {
        ...
        handler: async (response: any) => {
          try {
            ...
            // Money may well have left their account — never tell them to
            // just retry, and don't release the seat.
            setError(
              "We could not confirm your payment. If you were charged, contact support with this event name — do not pay again."
            );
          } finally {
            setLoading(false);
          }
        ...
        modal: {
          ondismiss: () => {
            // Give the seat back instead of leaving an unpaid row holding it.
            if (registrationId) releaseAbandoned(registrationId);
            setLoading(false);
          },
        },
      };
      ...
      if (registrationId) await releaseAbandoned(registrationId);
...
function ParticipantDashboard() {
  ...
  const fetchRegistrations = () => {
    apiList<Registration>("/api/registrations/mine")
  ...
  const maxSpend        = registrations.reduce((m, r) => Math.max(m, r.amount), 0);
        ...
        <div className="p-8 space-y-8">
          ...
          {loadError && (
            <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 text-sm flex items-center justify-between gap-4">
              <span>{loadError}</span>
              <button onClick={fetchEvents} className="underline whitespace-nowrap">Retry</button>
            </div>
          )}
                    ...
                    {events.map((ev, i) => (
                      <tr key={getId(ev)} className={`border-b border-white/[0.04] hover:bg-white/[0.02] transition-colors ${i === events.length - 1 ? "border-none" : ""}`}>
                        ...
                        <td className="px-6 py-4">
                          ...
                          >
                            {ev.availableSeats === 0 ? "Full" : (ev.price ?? 0) === 0 ? "Register Free" : "Register & Pay"}
                          </button>
                        </td>
                      </tr>
                    ))}
              ...
              <div className="divide-y divide-white/[0.04]">
                {registrations.length === 0 ? (
                  <div className="px-6 py-8 text-center text-muted text-sm">
                    No registrations yet 🎟️
                  </div>
                ) : (
                  registrations.map((reg) => (
                    <div key={getId(reg)} className="flex items-center gap-3 px-6 py-4 hover:bg-white/[0.02] transition-colors">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <button key={action.label}
                    onClick={action.label === "Edit Profile" ? () => setShowEditProfile(true) : undefined}
            ...
            <div className="bg-surface border border-white/[0.07] rounded-2xl p-6">
              ...
              <div className="space-y-4">
                {registrations.length === 0 ? (
                  <div className="text-center text-muted text-sm py-4">No payments yet</div>
                ) : (
                  registrations.map((reg) => (
                    <div key={getId(reg)}>
                      ...
                      {/* Bar widths are scaled against the biggest payment in the
                          list, so they compare to each other rather than to a
                          magic 1500. */}
                      <div className="h-2 bg-white/[0.05] rounded-full overflow-hidden">
                        <div
                          ...
                          style={{ width: `${reg.amount === 0 || maxSpend === 0 ? 100 : (reg.amount / maxSpend) * 100}%` }}
                        />
                      </div>
                    </div>
                  ))
