...
function AddServiceModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  const handleSubmit = async () => {
    // `!form.price` rejected "0", so a free service could never be listed.
    ...
    setError("");
    try {
      // vendorId comes from the verified session on the server — sending it
      // from here was both redundant and spoofable.
      await api("/api/vendors/services", {
        method: "POST",
        body: JSON.stringify({
          name:        form.name,
...
function EditServiceModal({ service, onClose, onUpdated }: { service: VendorService; onClose: () => void; onUpdated: () => void }) {
  ...
  const handleUpdate = async () => {
    ...
    setError("");
    try {
      await api(`/api/vendors/services/${service._id}`, {
        method: "PUT",
        body: JSON.stringify({
          name:        form.name,
          description: form.description,
      ...
      setError(err instanceof Error ? err.message : "Could not update the service.");
    } finally {
      setLoading(false);
    }
  };
...
function DeleteServiceModal({ service, onClose, onDeleted }: { service: VendorService; onClose: () => void; onDeleted: () => void }) {
  ...
  const handleDelete = async () => {
    try {
      await api(`/api/vendors/services/${service._id}`, { method: "DELETE" });
      onDeleted();
      onClose();
    } catch (err) {
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center">
      <div className="bg-surface border border-white/[0.1] rounded-2xl p-8 w-full max-w-sm shadow-2xl">
        ...
        <p className="text-muted text-sm mb-6">
          <span className="text-white font-medium">{service.name}</span> will be permanently deleted.
        </p>
        {error && (
          <div className="mb-4 p-3 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 text-xs">{error}</div>
        )}
...
function VendorListings() {
  ...
  const fetchServices = () => {
    setLoading(true);
    apiList<VendorService>(`/api/vendors/${vendorId}/services`)
      .then(setServices)
      .catch((err) => setLoadError(err instanceof Error ? err.message : "Could not load your listings."))
          ...
          <div className="bg-surface border border-white/[0.07] rounded-2xl overflow-hidden">
            ...
            {loading ? (
              ...
              <div className="text-center py-16">
                ...
                <div className="text-muted text-sm">No services listed yet</div>
                <button onClick={() => setShowAdd(true)}
