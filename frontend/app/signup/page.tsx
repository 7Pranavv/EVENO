...
const handleOAuth = async (provider: "google" | "microsoft" | "apple") => {
  ...
  const handleEmailSignup = async () => {
    ...
    try {
      ...
      if (resp.ok) {
        const userId = resp.data?.user?.userId || email;
        ...
        saveAuth(sessionJwt, {
          ...
          college: role === "organizer" ? college : undefined,
        });
        try {
          await fetch(`${API_BASE}/api/users/sync`, {
            method: "POST",
            headers: { "Content-Type": "application/json", Authorization: `Bearer ${sessionJwt}` },
            body: JSON.stringify({ name: name || email, email, role, college: role === "organizer" ? college : "" }),
          });
        } catch {
          // non-blocking
        }
